...
report = {"scope":"Synthetic single-task pairs, not general savings or model accuracy", "runs":[]}
reqs=[]
 ...
 for model in ["laya-astra","laya-sol"]:
  for arm,context in [("full",raw),("handoff",compact)]:
   ...
   try:
    ...
    row={"model":model,"arm":arm,"returncode":p.returncode,"elapsed_seconds":time.perf_counter()-start,"usage":usage,"correct": "AUTH-17" in answer and "deploy" in answer.lower(),"answer":answer,"stats_emitted":"Laya task stats:" in p.stdout+p.stderr}
